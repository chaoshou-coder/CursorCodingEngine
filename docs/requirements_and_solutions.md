...
| 能力 | 含义 | 实现载体 |
|------|------|----------|
| **编排能力** | 把复杂任务拆成模型能处理的粒度，按依赖并行执行 | codingengine DAG + planner |
...
| NFR2 | 利用已有技术 | codingengine 为独立产品，参考 simplerig/ECC/OMO/OpenSpec |
...
- **技术栈**：Python（codingengine）+ Cursor + Markdown（Skill/Rules）
- **参考项目**：simplerig、ECC、OMO、OpenSpec、Anthropic code-simplifier（只读参考，非依赖）
...
用户需求 → Cursor Agent 读 SKILL.md
         → 调用 codingengine init/emit
         → codingengine: plan → develop(DAG 并行) → verify
         → 每 task: 修改 → lint → code-simplifier → 原子提交
         → 失败: 诊断 → revert → 重试
```

**设计决策**：
- **codingengine** 为独立编排工具（事件溯源、DAG、断点续传），参考 simplerig 设计
...
| 组件 | 职责 | 改动 |
|------|------|------|
| src/codingengine/config.py | 模型与项目配置 | 废弃 performance_degradation_point、optimal_context，仅保留 context_limit |
| config.yaml | 配置源 | 同上 |
| src/codingengine/planner.py | 任务拆分 | 使用 context_limit 作为拆分依据；plan.json 增加 atomic_commit、rollback_strategy、dependencies、decoupled |
| src/codingengine/git_ops.py | Git 原子操作 | 原子提交（task_id 标记）、失败回滚（git revert） |
| src/codingengine/runner.py | 阶段执行 | 集成 git_ops，每任务执行后原子提交 |
| .cursor/skills/codingengine/SKILL.md | 主工作流 | 融入 OMO Just Do、Boulder 续航、ECC 质量模式、Wisdom 积累 |
...
```
plan 阶段
  ├─ codingengine emit stage.started --stage plan
  ├─ 按 context_limit 拆分任务 → plan.json
  └─ codingengine emit stage.completed --stage plan

develop 阶段
  ├─ codingengine emit stage.started --stage develop
  ├─ DAG 并行：task_1, task_2, ... (无依赖可并行)
  │   └─ 每 task: 修改 → lint → code-simplifier → git commit (atomic)
  └─ codingengine emit stage.completed --stage develop

verify 阶段
  ├─ codingengine emit stage.started --stage verify
  ├─ 全量 lint + test
  ├─ 失败 → 诊断 → git revert 失败 task → 重试 (最多 3 轮)
  └─ 通过 → wisdom 记录 → codingengine emit run.completed
```
...
| 60+ Skill | 过度复杂，逻辑并入 codingengine SKILL |
| ECC Hook 适配层 | Rules 足够 |
| OpenSpec CLI 集成 | codingengine plan.json 已覆盖 |
| 多平台适配 | Cursor only |
...
│   └── skills/
│       ├── codingengine/SKILL.md    # 主工作流
│       └── code-simplifier/SKILL.md
├── src/codingengine/             # 独立产品
│   ├── config.py
│   ├── planner.py
│   ├── runner.py
│   ├── git_ops.py
...
| 步骤 | 任务 | 产出 |
|------|------|------|
| 1 | 安装 codingengine | pip install -e . |
...
| `docs/wiki/` | Wiki 文档集（Home、Getting-Started、CLI-Reference 等） |
