...
- 仅在图形界面人工验证、业务决策需拍板、外部权限/数据缺失时，才进入等待人工
...
- **必须激活项目 venv**：执行 pip/codingengine 前先激活。创建 venv 时建议命名 `CCEfor<项目名>`（如 `CCEforEagle`），便于区分多项目环境
- **不污染项目目录**：所有数据在 `~/.codingengine/runs/`，项目内零新增文件

## 准备阶段

1. 进入项目根目录，激活 venv（建议命名 CCEfor<项目名>）
