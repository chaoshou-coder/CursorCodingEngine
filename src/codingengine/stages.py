...
def develop_handler(ctx: StageContext) -> StageResult:
    ...
    if tdd_mode and plan:
        tdd_test = plan.get("tdd_test_file") or (
            plan.get("tasks")
            and next(
                (t.get("test_file") for t in plan["tasks"] if t.get("test_file")), None
            )
        )
        tdd_impl = plan.get("tdd_impl_file") or (
            plan.get("tasks")
            and next(
                (t.get("impl_file") for t in plan["tasks"] if t.get("impl_file")), None
            )
        )
        if tdd_test and tdd_impl:
            try:
                ...
                test_path = (
                    project_root / tdd_test
                    if not Path(tdd_test).is_absolute()
                    else Path(tdd_test)
                )
                impl_path = (
                    project_root / tdd_impl
                    if not Path(tdd_impl).is_absolute()
                    else Path(tdd_impl)
                )
                runner = TDDRunner(ctx.config, ctx.writer, ctx.run_id)
                result = runner.run_cycle(
                    test_path, impl_path, lambda: None, project_root=project_root
                )
    ...
    changes = {
        ...
        "changes": [
            {"file": "stub_file.py", "action": "create", "content": "# Stub\n"}
        ],
        "stub": not tdd_ran,
    }
    ...
    if tdd_ran and not tdd_success:
        ...
        return StageResult(
            status=StageStatus.FAILED, outputs=[ref], error="TDD cycle failed"
        )
...
def verify_handler(ctx: StageContext) -> StageResult:
    ...
    bdd_result = None
    feature_files = (
        list(Path(ctx.store.artifacts_dir).glob("*.feature"))
        if hasattr(ctx.store, "artifacts_dir") and ctx.store.artifacts_dir.exists()
        else []
    )
...
def integrate_handler(ctx: StageContext) -> StageResult:
    ...
    if verify_result and not verify_result.get("overall_success", False):
        return StageResult(
            status=StageStatus.FAILED, error="Cannot integrate: verification failed"
        )
    ref = ctx.store.write(
        "integration_result.json",
        {
            ...
            "integration": {"applied_changes": [], "commit_hash": None},
            "stub": True,
        },
    )
    ctx.writer.emit("artifact.written", ctx.run_id, artifact=ref.to_dict())
