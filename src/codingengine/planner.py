...
class Planner:
    ...
    def plan_from_architecture(self, architecture_doc: str) -> List[AtomicTask]:
        modules = [
            l.strip()
            for l in architecture_doc.split("\n")
        ...
        for i, module in enumerate(modules):
            ...
            for t in tasks:
                ...
                if any(k in module.lower() for k in kw if len(k) > 2):
                    deps.append(t.id)
            group = (
                max((t.parallel_group for t in tasks if t.id in deps), default=0) + 1
                if deps
                else 0
            )
            if estimated > self.safe_limit:
                ...
                lines_per = max(
                    1,
                    self.safe_limit // (len(module) // 4 // len(lines)) if lines else 1,
                )
    ...
    def export_plan(self, tasks: List[AtomicTask]) -> Dict:
        ...
        return {
            ...
            "parallel_groups": [
                [t.id for t in tasks if t.parallel_group == g] for g in groups
            ],
