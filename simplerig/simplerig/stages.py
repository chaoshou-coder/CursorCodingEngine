...
def develop_handler(ctx: StageContext) -> StageResult:
    ...
    if tdd_ran and not tdd_success:
        # 失败时按 plan 的 rollback_strategy 回滚
        if plan and plan.get("rollback_strategy") == "git_revert":
            ok, err = rollback_task_commit(project_root, f"develop_{ctx.run_id}")
            if not ok and err:
                ctx.writer.emit("rollback.failed", ctx.run_id, error=err)
        return StageResult(
    ...
    # 成功时：若 plan 启用 atomic_commit 且有未提交变更，执行原子提交
    if plan and plan.get("atomic_commit") and has_uncommitted_changes(project_root):
        ok, err = atomic_commit(project_root, f"develop_{ctx.run_id}")
        if not ok and err:
            ctx.writer.emit("commit.failed", ctx.run_id, error=err)
